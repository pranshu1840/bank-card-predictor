...
def index_func(request):
    ...
    if request.method == 'POST':
        ...
        if name != "":
            ...
            df2 = pd.DataFrame([{'Attrition_Flag': int(attrite), 'Customer_Age': float(age), 'Gender': int(gender),
                                 'Dependent_count': float(children), 'Education_Level': int(edu), 'Marital_Status': int(marital),
                                 'Income_Category': int(income), 'Months_on_book': int(monthOnBooks),
                                 'Total_Relationship_Count': int(totalRelationshipCount), 'Months_Inactive_12_mon': int(monthsInactive12mon),
                                 'Contacts_Count_12_mon': int(contactsCount12mon), 'Credit_Limit': float(creditLimit),
                                 'Total_Revolving_Bal': float(totalRevolvingBal), 'Avg_Open_To_Buy': float(avgOpenToBuy),
                                 'Total_Amt_Chng_Q4_Q1': float(totalAmtChngQ4Q1), 'Total_Trans_Amt': float(totalTransAmt),
                                 'Total_Trans_Ct': float(totalTransCt), 'Total_Ct_Chng_Q4_Q1': float(totalCtChngQ4Q1),
                                 'Avg_Utilization_Ratio': float(avgUtilizationRatio)}])

            # Define the columns of the DataFrame
            columns = ['Attrition_Flag', 'Customer_Age', 'Gender', 'Dependent_count', 'Education_Level', 'Marital_Status',
                       'Income_Category', 'Months_on_book', 'Total_Relationship_Count', 'Months_Inactive_12_mon',
                       'Contacts_Count_12_mon', 'Credit_Limit', 'Total_Revolving_Bal', 'Avg_Open_To_Buy',
                       'Total_Amt_Chng_Q4_Q1', 'Total_Trans_Amt', 'Total_Trans_Ct', 'Total_Ct_Chng_Q4_Q1', 'Avg_Utilization_Ratio']

            # Create the initial empty DataFrame with the specified columns
